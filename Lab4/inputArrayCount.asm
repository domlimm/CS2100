...
array:
    # code for reading in the array values
    slt   $t2, $s1, $s2                     # if last element of arrayA (store)
    beq   $t2, $0, reinit                   # if last element, goto br reinit 

    # code for reading in the array values
    li    $v0, 4                            # system call code for print_string
    la    $a0, value                        # address of input line
    syscall

    li    $v0, 5                            # system call code for read_int
    syscall

    la    $t2, 0($v0)                       # store value into $t2
    sw    $t2, 0($s1)                       # store value into arrayA

    addi  $s1, $s1, 4                       # move to next element of arrayA
    j     array

reinit:
    # code for reinitialising $s1
    addi  $s1, $t0, 0                       # start index of arrayA
